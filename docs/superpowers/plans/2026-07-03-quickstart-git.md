...
Expected: `8` (one `#` title + six `##` sections, including "You don't have to edit raw markers by hand" + one `# ...make commits...` bash comment line inside a code block, which also starts with `#`)
